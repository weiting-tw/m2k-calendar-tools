...
(function () {
  ...
  async function loadDepts() {
    ...
    await grab("");           // 頂層 + 已展開者
    await grab("/ORG_DIR1");    // 公司員工部門樹（BMD/UNIT1/…）
    return (DEPTS = depts);
  }
  ...
  function buildPanel() {
    ...
    panel.innerHTML = `
      ...
      <div style="display:flex;gap:4px">
        ...
        <input id="gb-q" placeholder="人名 / 部門(如 UNIT1)" style="flex:1;min-width:0">
    ...
    async function doDeptSearch(q) {
      ...
      try {
        ...
        if (!matches.length) { res.innerHTML = "查無此部門。可試 ORG_DIR1 底下的代碼，如 UNIT1。（純郵件群組無法展開）"; return; }
        res.innerHTML = "";
        matches.forEach((d) => {
    ...
    $("#gb-mode").addEventListener("change", () => { res.style.display = "none"; res.innerHTML = ""; $("#gb-q").placeholder = $("#gb-mode").value === "person" ? "打名字，會即時跳建議" : "部門代碼(如 UNIT1) 後按搜尋"; });
