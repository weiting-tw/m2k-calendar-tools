...
- **通訊錄（adb2）**:同源、只靠 cookie。
  ...
  - 部門樹：`/cgi-bin/adb2tree_mds`(`command=expand`,需帶 `workingabid`)，路徑如 `/GSS_EMP/CSBDBG`。
  - 人員搜尋：`/cgi-bin/adb2search_mds`(`command=mdssearch`)。
  - **純郵件群組（distribution list，如 cs_pd3_csbdbg@）無法展開**——成員在郵件伺服器端，通訊錄查不到。
...
2. **純郵件群組展不開**（cs_pd3_csbdbg 這類）——資料源不開放，無解。
