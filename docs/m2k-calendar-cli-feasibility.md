...
- 你的帳號 principal 路徑：`/cgi-bin/cal/caldav/principals/wilber_chen@gss.com.tw/`
...
| 帳號 | `wilber_chen@gss.com.tw` |
| principal | `/cgi-bin/cal/caldav/principals/wilber_chen@gss.com.tw/` |
...
1. **標準 CardDAV（乾淨但目前無資料）**：`/cgi-bin/carddav/` 有開（`addressbook`），你的通訊錄家目錄是 `/cgi-bin/carddav/addressbooks/wilber_chen@gss.com.tw/`，但底下**沒有任何通訊錄被佈建**，公司全域通訊錄（GAL）也沒透過 CardDAV 暴露。`principal-property-search` 在根層也查不到人。→ 這條路現在拿不到同事資料。
...
- 目錄：`個人通訊錄`、**`GSS`**、**`GSS_ALL`**（全公司）、`虛擬目錄`(Backend/Front/release…)
...
**已實測成功（用你登入的瀏覽器）**：GSS → GSS_PT 部門直接列出成員，每筆含**姓名（中英）＋信箱＋分機**（email 格式 `英文名_姓@gss.com.tw`），有分頁（每頁 25 筆）。上方工具列的「加入與會者」按鈕證實這個選擇器正是行事曆加與會者用的。→ 群組展開技術上完全可行。
...
| 通訊錄結構 | 公司：`GSS`→`GSS_EMP`/`GSS_PT`(部門樹)、`GSS_ALL`(全公司)；個人：`個人通訊錄`+虛擬群組 |
| 每頁 | 25 筆，需翻頁 |
